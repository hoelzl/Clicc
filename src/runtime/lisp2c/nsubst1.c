...
static void Z17_nsubst_internal(CL_FORM *base, CL_FORM *display[]);
static void Z18_lambda(CL_FORM *base, int nargs);
...
void nsubst1(CL_FORM *base)
{
	...
	else
	{
		...
		{
			GEN_CLOSURE(array, ARG(6), 4, Z18_lambda, -1);
			COPY(ARG(4), &array[3]);
	...
	COPY(ARG(2), ARG(6));
	display[0] = ARG(0);
	Z17_nsubst_internal(ARG(6), display);
	COPY(ARG(6), ARG(0));
}
...
static void Z17_nsubst_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		else
		{
			...
			Z17_nsubst_internal(ARG(1), display);
			COPY(GET_CDR(ARG(0)), ARG(2));
			Z17_nsubst_internal(ARG(2), display);
