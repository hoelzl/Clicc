...
static void Z128_mapcan_internal(CL_FORM *base, CL_FORM *display[]);
...
void Fmapcan(CL_FORM *base, int nargs)
{
	...
	Flist(ARG(2), nargs - 2);
	display[0] = ARG(0);
	Z128_mapcan_internal(ARG(3), display);
	COPY(ARG(3), ARG(0));
}

static void Z129_get_rest_args(CL_FORM *base, CL_FORM *display[]);
...
static void Z128_mapcan_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	if(caller_base == NULL)
	{
		...
		else
		{
			...
			display[1] = ARG(0);
			Z129_get_rest_args(ARG(3), display);
			Fapply(ARG(1), 3);
			mv_count = 1;
			Z128_mapcan_internal(ARG(2), display);
			Fnconc(ARG(1), 2);
			COPY(ARG(1), ARG(0));
		}
		RETURN1:;
...
static void Z129_get_rest_args(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		Z129_get_rest_args(ARG(2), display);
