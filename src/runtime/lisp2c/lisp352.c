...
void Fhost_namestring(CL_FORM *base)
{
	...
	else
	{
		...
		else
		{
			...
			{
				LOAD_SMSTR((CL_FORM *)&KClisp[122], ARG(1));	/* etypecase: the value ~a is not a legal value */
				COPY(ARG(0), ARG(2));
				Ferror(ARG(1), 2);
			}
		}
	...
	if(CL_TRUEP(ARG(2)))
	{
		...
		if(CL_TRUEP(ARG(5)))
		{
			COPY(OFFSET(AR_BASE(GET_FORM(ARG(4))), 2 + 1), ARG(3));
		}
		else
		{
	...
	{
		LOAD_SMSTR((CL_FORM *)&KClisp[120], ARG(0));	/* Cannot determine the namestring for pathnames with no host:~%  ~S */
		Ferror(ARG(0), 2);
	}
}
