...
static void Z124_mapcar_internal(CL_FORM *base, CL_FORM *display[]);
...
void Fmapcar(CL_FORM *base, int nargs)
{
	...
	Flist(ARG(2), nargs - 2);
	display[0] = ARG(0);
	Z124_mapcar_internal(ARG(3), display);
	COPY(ARG(3), ARG(0));
}

static void Z125_get_rest_args(CL_FORM *base, CL_FORM *display[]);
...
static void Z124_mapcar_internal(CL_FORM *base, CL_FORM *display[])
{
	...
	if(caller_base == NULL)
	{
		...
		else
		{
			...
			display[1] = ARG(0);
			Z125_get_rest_args(ARG(3), display);
			Fapply(ARG(1), 3);
			mv_count = 1;
			Z124_mapcar_internal(ARG(2), display);
...
static void Z125_get_rest_args(CL_FORM *base, CL_FORM *display[])
{
	...
	else
	{
		...
		Z125_get_rest_args(ARG(2), display);
		ALLOC_CONS(ARG(3), ARG(1), ARG(2), ARG(0));
	}
}
